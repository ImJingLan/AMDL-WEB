...
func loadConfig() error {
	// 从标准输入 os.Stdin 读取所有数据
	data, err := ioutil.ReadAll(os.Stdin)
	if err != nil {
		// 如果从 stdin 读取失败，返回包含上下文的错误
		return fmt.Errorf("从标准输入读取配置失败: %w", err)
	}

	// 检查读取到的数据是否为空
	if len(data) == 0 {
		return errors.New("从标准输入读取到的配置数据为空")
	}

	// 将从 stdin 读取到的 yaml 数据解析到 Config 变量中
	err = yaml.Unmarshal(data, &Config)
	if err != nil {
		// 如果 YAML 解析失败，返回包含上下文的错误
		// 同时打印读取到的数据（截断），帮助调试
		log_data_preview := string(data)
		if len(log_data_preview) > 500 {
			log_data_preview = log_data_preview[:500] + "..." // 截断过长的输入内容
		}
		return fmt.Errorf("解析从标准输入传入的 YAML 配置失败: %w\n接收到的数据预览:\n%s", err, log_data_preview)
	}
...
func downloadTrack(trackNum int, trackTotal int, meta *structs.AutoGenerated, track structs.TrackData, albumId, token, storefront, mediaUserToken, sanAlbumFolder, Codec string, counter *structs.Counter) {
	...
	if len(mediaUserToken) > 50 {
		...
		if err != nil {
			...
			fmt.Printf("SPECIFIC_LYRICS_FAILURE: %s\n", track.ID) // 新增特定标记输出
...
func writeMP4Tags(trackPath string, meta *structs.AutoGenerated, trackNum, trackTotal int) error {
	...
	if strings.Contains(meta.Data[0].ID, "pl.") && !Config.UseSongInfoForPlaylist {
		...
		for _, track := range meta.Data[0].Relationships.Tracks.Data {
			if track.Attributes.DiscNumber > maxDiscNumber {
				maxDiscNumber = track.Attributes.DiscNumber
			}
		}
		// 只有在有多张光碟时才设置碟片编号
		if maxDiscNumber > 1 {
			t.DiscNumber = int16(meta.Data[0].Relationships.Tracks.Data[index].Attributes.DiscNumber)
			t.DiscTotal = int16(maxDiscNumber)
		}
...
func main() {
	...
	pflag.BoolVar(&skip_check, "skip-check", false, "Skip file existence check")
...
func getToken() (string, error) {
	// 1. 优先使用从 Python 动态传入的 ApiToken
	if Config.ApiToken != "" {
		// log.Println("Using API token dynamically provided by Python.") // 可选的调试日志
		return Config.ApiToken, nil
	}

	// 2. 其次，检查用户是否在 source.yaml 中静态配置了 AuthorizationToken
	// 注意：原代码中 authorization-token 的检查逻辑似乎不完整，这里假设它是用户可能在 source.yaml 中设置的
	if Config.AuthorizationToken != "" && Config.AuthorizationToken != "your-authorization-token" { // "your-authorization-token" 通常是占位符
		// log.Println("Using static AuthorizationToken from source.yaml.") // 可选的调试日志
		staticToken := strings.Replace(Config.AuthorizationToken, "Bearer ", "", -1)
		return staticToken, nil
	}

	// 3. 如果以上都没有，则从网站获取
	// log.Println("No pre-configured token found, fetching new token from Apple Music website.") // 可选的调试日志
	...
	if indexJsUri == "" {
		return "", errors.New("failed to find index-legacy JS URI in HTML body")
	}
	...
	if token == "" {
		return "", errors.New("failed to find token in JS file")
	}
	return token, nil
}
