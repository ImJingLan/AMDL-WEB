...
func Run(adamId string, playlistUrl string, outfile string, Config structs.ConfigSet) error {
	...
	if optstimeout > 0 {
		...
		if do.ContentLength < int64(Config.MaxMemoryLimit * 1024 * 1024) {
			...
			totalSize := do.ContentLength
			fmt.Printf("DL_PROGRESS:0/%d\n", totalSize)
			
			// 更细粒度的缓冲区，使写入更频繁
			smallChunkBuffer := &smallChunkWriter{
				Writer: &buffer,
				ChunkSize: 8192, // 使用8KB作为输出频率的阈值
				OnWrite: func(n int) {
					downloaded += int64(n)
					fmt.Printf("DL_PROGRESS:%d/%d\n", downloaded, totalSize)
				},
			}
			
			io.Copy(io.MultiWriter(smallChunkBuffer, bar), do.Body)
			body = &buffer
...
func downloadAndDecryptFile(conn io.ReadWriter, in io.Reader, outfile string,
	...
	inBuf := bufio.NewReader(progressReader)
...
// 添加一个用于计数的Writer实现，每次写入时都输出进度
type countingWriter struct {
	io.Writer
	OnWrite func(p []byte, n int)
}
...
// 添加一个小块写入的Writer实现，更高频率地报告进度
type smallChunkWriter struct {
	io.Writer
	buffer []byte
	ChunkSize int
	OnWrite func(int)
...
func (w *smallChunkWriter) Write(p []byte) (n int, err error) {
	n, err = w.Writer.Write(p)
	if n > 0 && w.OnWrite != nil {
		w.OnWrite(n)
	}
